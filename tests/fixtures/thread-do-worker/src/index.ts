export class Counter {
	...
	async fetch(request: Request): Promise<Response> {
		...
		if (url.pathname === '/cookies') {
			// Two Set-Cookie headers — exercises header serialization across the
			// DO-worker → main bridge, which has to carry repeated names as separate
			// entries rather than folding them into a keyed record.
