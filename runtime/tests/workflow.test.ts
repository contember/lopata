...
import { NonRetryableError, parseDuration, SqliteWorkflowBinding, SqliteWorkflowInstance, WorkflowEntrypointBase } from '../bindings/workflow'
...
class EventWorkflow extends WorkflowEntrypointBase {
	override async run(
		...
		step: {
			...
			waitForEvent: <T>(name: string, options: { type: string; timeout?: string }) => Promise<{ payload: T; timestamp: Date; type: string }>
		},
	): Promise<unknown> {
...
class SleepUntilWorkflow extends WorkflowEntrypointBase {
	override async run(
		_event: unknown,
		step: { do: <T>(name: string, cb: () => Promise<T>) => Promise<T>; sleepUntil: (name: string, timestamp: Date) => Promise<void> },
	): Promise<unknown> {
...
class TimeoutEventWorkflow extends WorkflowEntrypointBase {
	override async run(
		_event: unknown,
		step: { waitForEvent: <T>(name: string, options: { type: string; timeout?: string }) => Promise<{ payload: T; timestamp: Date; type: string }> },
	): Promise<unknown> {
...
describe('waitForEvent / sendEvent', () => {
		...
		class LateEventWorkflow extends WorkflowEntrypointBase {
			override async run(
				...
				step: {
					...
					waitForEvent: <T>(name: string, options: { type: string }) => Promise<{ payload: T; timestamp: Date; type: string }>
				},
			): Promise<unknown> {
	...
	test('waitForEvent returns WorkflowStepEvent with payload, timestamp, type', async () => {
		class EventStructWorkflow extends WorkflowEntrypointBase {
			override async run(
				_event: unknown,
				step: { waitForEvent: <T>(name: string, options: { type: string }) => Promise<{ payload: T; timestamp: Date; type: string }> },
			): Promise<unknown> {
...
describe('step retry config', () => {
	test('step.do retries on failure with constant backoff', async () => {
		...
		class RetryWorkflow extends WorkflowEntrypointBase {
			override async run(
				_event: unknown,
				step: { do: <T>(name: string, config: WorkflowStepConfig, cb: () => Promise<T>) => Promise<T> },
			): Promise<unknown> {
				const result = await step.do(
					'flaky',
					{ retries: { limit: 3, delay: '10 milliseconds', backoff: 'constant' }, timeout: '5 seconds' },
					async () => {
						attempts++
...
describe('event instanceId', () => {
	test('event object contains instanceId', async () => {
		class InstanceIdWorkflow extends WorkflowEntrypointBase {
			override async run(
				event: { payload: unknown; instanceId: string },
...
describe('waitForEvent default timeout', () => {
	test('waitForEvent has default 24h timeout', async () => {
		...
		class DefaultTimeoutWorkflow extends WorkflowEntrypointBase {
			override async run(
				_event: unknown,
				step: { waitForEvent: <T>(name: string, options: { type: string }) => Promise<{ payload: T }> },
			): Promise<unknown> {
...
describe('waitForEvent duplicate step name', () => {
	test('throws on duplicate waitForEvent name', async () => {
		class DupWaitWorkflow extends WorkflowEntrypointBase {
			override async run(
				_event: unknown,
				step: { waitForEvent: <T>(name: string, options: { type: string; timeout?: string }) => Promise<{ payload: T }> },
			): Promise<unknown> {
...
describe('event timestamp on restart', () => {
	test('event.timestamp matches original created_at after restart', async () => {
		class TimestampWorkflow extends WorkflowEntrypointBase {
			override async run(
				event: { payload: unknown; timestamp: Date },
				step: { do: <T>(name: string, cb: () => Promise<T>) => Promise<T> },
			): Promise<unknown> {
