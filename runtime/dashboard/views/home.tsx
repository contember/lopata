        ...
        <div class="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {CARDS.map(card => (
            <a
              key={card.key}
              href={`#${card.path}`}
              class="block bg-panel rounded-lg border border-border p-5 no-underline hover:border-border transition-colors group"
            >
              <div class="flex items-center gap-3">
                <span class="text-3xl text-text-dim">{card.icon}</span>
                <div>
