...
import type { AnalyticsEngineDataPoint as AEDataPoint, HandlerContext, OkResponse } from '../types'

export const handlers = {
	'analyticsEngine.list'({ dataset, limit = 50 }: { dataset?: string; limit?: number }): AEDataPoint[] {
		const db = getDatabase()
		let query =
			'SELECT id, dataset, timestamp, index1, blob1, blob2, blob3, blob4, blob5, double1, double2, double3, double4, double5, _sample_interval FROM analytics_engine'
