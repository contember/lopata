			...
			<div class="overflow-y-auto flex-1 divide-y divide-border-row">
				{columns.map(col => {
					...
					if (value !== null && strVal.length > 0) {
						try {
							formatted = JSON.stringify(JSON.parse(strVal), null, 2)
							isJson = true
						} catch {}
					}
					return (
						<div key={col} class="px-5 py-3 flex gap-4 group">
							...
							<div class="flex-1 min-w-0 flex items-start gap-2">
								{value === null ? <span class="text-text-dim italic text-xs">NULL</span> : fk && onNavigateFK
									? (
										<button
											onClick={() => {
												onNavigateFK(fk.targetTable, fk.targetColumn, value)
												onClose()
											}}
