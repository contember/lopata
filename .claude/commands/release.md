...
1. **Fetch remote tags** to ensure local tags are up to date:

!git fetch --tags

2. **Find the last release tag and show changes since then:**

!git describe --tags --abbrev=0

!git log $(git describe --tags --abbrev=0)..HEAD --oneline

3. **Analyze the changes above and determine the version bump type:**
...
4. **Read current version from package.json** and calculate the new version.

5. **Update version in package.json**.

6. **Run `bun run build:assets`** to verify the build succeeds.

7. **Commit, push, and create the tag:**
   ```
