...
import { DeleteButton, EmptyState, PageHeader, RefreshButton, ServiceInfo, Table } from '../components'
		...
		<div class="p-4 sm:p-8 max-w-6xl">
			...
			<div class="flex flex-col lg:flex-row gap-6 items-start">
				<div class="flex-1 min-w-0">
					...
					{!points?.length ? <EmptyState message="No data points found" /> : (
						<Table
		...
		<div class="p-4 sm:p-8 max-w-4xl">
			...
			<div class="bg-panel border border-border rounded-lg p-6">
				...
				<div class="grid grid-cols-2 gap-4 mb-4 text-sm">
					<div>
						<span class="text-text-muted">Dataset:</span> <span class="font-mono">{data.dataset}</span>
					</div>
					<div>
						<span class="text-text-muted">Time:</span> {formatTime(data.timestamp)}
					</div>
					<div>
						<span class="text-text-muted">Index:</span> <span class="font-mono">{data.index1 ?? '-'}</span>
					</div>
					<div>
						<span class="text-text-muted">Sample interval:</span> <span class="tabular-nums">{data._sample_interval}</span>
					</div>
				</div>
				{doubles.length > 0 && (
					<div class="mb-4">
						...
						<div class="bg-panel-secondary border border-border rounded-lg p-4">
							<div class="grid grid-cols-4 gap-2 text-sm">
								{doubles.map(d => (
									<div key={d.name}>
										<span class="text-text-muted text-xs">{d.name}:</span> <span class="font-mono tabular-nums">{d.value}</span>
									</div>
								))}
							</div>
						</div>
					...
					<div>
						...
						<div class="bg-panel-secondary border border-border rounded-lg p-4">
							<div class="space-y-1 text-sm">
								{blobs.map(b => (
									<div key={b.name}>
										<span class="text-text-muted text-xs">{b.name}:</span> <span class="font-mono">{b.value}</span>
									</div>
								))}
							</div>
						</div>
