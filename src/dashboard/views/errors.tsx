...
import type { ErrorDetail, ErrorSummary, SpanData } from '../rpc/types'
	...
	return (
		<div class="p-8 h-full flex flex-col">
			...
			<div class="flex-1 overflow-y-auto scrollbar-thin">
										...
										>
											...
											<td class="px-4 py-2.5 text-text-secondary font-mono text-xs">
												{err.requestMethod && err.requestUrl
													? (
														<span>
															<span class="font-medium">{err.requestMethod}</span> {truncateUrl(err.requestUrl)}
														</span>
													)
				...
				{isLoading && errors.length === 0 && <div class="text-text-muted text-sm text-center py-12">Loading errors...</div>}
			</div>
		</div>
	)
}
		...
		<div class="p-6 max-w-6xl flex flex-col gap-4">
				...
				<div class="px-5 py-4">
					<div class="flex items-center gap-2.5 mb-1.5">
						<span
							...
							style={{ background: 'var(--color-error-icon-bg)', color: 'var(--color-error-icon-text)' }}
						>
							!
						</span>
			...
			{data.bindings.length > 0 && (
				<Section title="Bindings">
					{data.bindings.length === 0 ? <div class="px-4 py-3 text-sm text-text-muted">No bindings configured</div> : (
						<table class="w-full text-sm">
							<tbody>
								{data.bindings.map((b) => (
